...
const Mountain = () => {
  ...
  return (
    <group>
      ...
      <RigidBody type="fixed" colliders="trimesh" position={[-170, -75, 0]} rotation={[0, Math.PI /2, 0]}>
        <primitive object={model.scene.clone()} scale={2} />
      </RigidBody>
      <RigidBody type="fixed" colliders="trimesh" position={[-150, -80  , 0]} rotation={[0,0,Math.PI /2]}>
        <primitive object={model.scene.clone()} scale={2} />
      </RigidBody>
    </group>
  );
