...
import { Cloud, Html, PerspectiveCamera, Sky, Stars } from "@react-three/drei";
...
const Lights = ({ intensity }) => {
  return (
    <>
      <ambientLight intensity={intensity === 0 ? 0.1 : intensity} />
      {/* <directionalLight
        ref={lightRef}
        position={[0, 10, 10]}
        castShadow
      ...
      /> */}
      <hemisphereLight
        args={["lightorange", "#000", intensity === 0 ? 0.1 : 0.3]}
      />
    </>
  );
};
...
const App = () => {
  ...
  const [enviroment, setEnviroment] = useState({ type: "day", inclination: 1 });
  ...
  const handleEnviroment = () => {
    ...
    const countdown = setInterval(() => {
      ...
      if (initial < 0.4) {
        ...
        setEnviroment({ type: "day", inclination: 1 });
        clearInterval(countdown);
      }
    }, 100);
  };
  ...
  return (
    <Suspense>
      ...
      <Sky
        // sunPosition={[0, 0.001, 0]}
        rayleigh={0.6}
        mieCoefficient={0.095}
